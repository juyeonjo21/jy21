...
public class Test01 {
	public static void main(String[] args) {
		...
		//파일 정보 분석
		System.out.println(a.exists()); // 진짜 있는 파일이냐?
		System.out.println(b.exists());
		
		System.out.println(a.isDirectory()); //디렉터리인지 확인
		
		System.out.println(a.isFile()); // 파일인지 확인
		
		if(a.isFile()) { //파일이라면
			System.out.println(a.getName()); //이름 뭐야
			System.out.println(a.getPath()); //위치가 어디야
			System.out.println(a.length()); // 파일의 크기(=글자수) -> 반환형이 Long이다
