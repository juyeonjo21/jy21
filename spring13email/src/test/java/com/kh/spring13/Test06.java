...
public class Test06 {
	...
	public void test() throws MessagingException, IOException {
		//목표 : 외부에 만들어 놓은 파일에 있는 HTML을 불러와서 전송(/templates/email.html)
		MimeMessage message = sender.createMimeMessage();
		MimeMessageHelper helper = new MimeMessageHelper(message, false, "UTF-8");
		
		helper.setTo("merry2j@naver.com");
		helper.setSubject("이메일 템플릿 전송 예제");
		
		//classpath에 있는 templates 폴더의 email.html을 불러오는 코드
		ClassPathResource resource = new ClassPathResource("templates/email2.html");//classpath의 자원 선택
...
//		text = text.replace("#1", "바꿀키워드"); //비추천
		...
		link.attr("href","https://www.goolge.com"); //가져온 태그의 href 속성을 변경해라!
