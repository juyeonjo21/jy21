...
/**
 	...
 	만드는 법
 	1.상속을 받아서 자격을 획득(implements HandlerInterceptor)
 	2.등록(@component)
 	3.메소드 재정의
...
public class TestInterceptor implements HandlerInterceptor{
	...
	/**
	 ...
	 - true를 반환하면 요청에 대한 작업을 '진행'하겠다는 의미
	 - false를 반환하면 요청에 대한 작업을 '차단'하겠다는 의미
	 - 자격 여부에 따른 접근 가능/차단 여부를 처리할 때 적합하다.
	 - request에는 사용자의 요청 정보가 들어있다.(ex-파라미터,세션정보,...등)
	 - response에는 사용자에게 나갈 정보가 들어있다.(ex-화면정보,...등)
	 - Handler에는 이 요청을 처리할 대상 컨트롤러/메소드의 정보가 들어있다.
	 */
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		throws Exception{ //컨트롤러 실행 전
	...
	/**
	 ...
	 - 유일하게 예외의 발생 여부를 알 수 있다(문제점 확인 가능)
	 - 발생한 예외에 따라 처리할 내용들을 이곳에 작성할 수 있다.
	 - 사용자가 볼 페이지를 변경할 수는 없다. (문제는 확인 가능 - 처리는 불가)
	 								(모니터링,로깅-기록을 남기는 행위,...)
	 */
	@Override
	public void afterCompletion(HttpServletRequest request, 
			...
			Exception ex)
		throws Exception {
	System.out.println("efterCompletion 실행");
