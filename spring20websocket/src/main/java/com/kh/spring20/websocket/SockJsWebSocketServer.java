...
public class SockJsWebSocketServer extends TextWebSocketHandler {
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		// 모든 접속자에게 접속자 명단을 전송
		// 메소드 추가
		sendClientList();//전체에게 전송
		sendMessageList(client);//접속자에게 메세지 내역 전송
	}
	...
	public void sendMessageList(ClientVO client) throws IOException {
		...
		for(ChatDto dto : list) {
			...
			//- dto에 chatReceiver가 있으면 DM, 없으면 DM 아님
			//- 시간은 FE 미구현으로 첨부하지 않음
			
			if(dto.getChatReceiver() == null) {//전체 채팅인 경우 - chatReceiver가 null인 경우
				...
				//접속한 사람이 보낸 메세지라면 5개의 데이터를 전송(dm,memberId,memberLevel,content,chatReceiver)
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		ClientVO client = new ClientVO(session);
		if (client.isMember() == false) return; // 멤버가 아니면 메세지 못 봄
		
		//(+추가) 사용자는 메세지를 JSON 형태로 보내므로 이를 해석해야 한다(ObjectMapper)
		Map params = mapper.readValue(message.getPayload(), Map.class);//해석 코드
