...
public class FileUtil {
	...
	//		[1] 대상 위치 안에 원본 디렉터리(폴더) 이름과 같은 디렉터리를 생성
	...
	public static void copy(File origin, File target) throws IOException {
		if(origin.isFile()) {
			...
			//target이 없으면 생성
			if(target.exists()){
				target.mkdirs(); // D:/test 면
			}
			//같은 이름으로 target 아래에 생성
			File dir = new File(target,origin.getName()); //"D:/test",원본폴더명
			dir.mkdirs();
	...
	//파일&폴더 삭제 메소드
	// - 파일일 경우는 그냥 삭제하면 된다
	// - 디렉터리일 경우는 내부에 있는 모든 요소를 삭제한 뒤 삭제해야한다
	// 	- 내부에 있는 요소는 파일인지 티렉터리인지 알 수 없다
	// 	- 다시 검사해서 삭제하도록 요청해야 한다(재귀, recursive)
	public static void delete(File target) {
		if(target.isFile()) {
			target.delete();	//파일은 그냥 삭제
		}
		else if(target.isDirectory()){		//디렉터리는 내용물을 지우고 
			File[] list = target.listFiles();
			for(File file : list) {
				delete(file); //재귀 호출(같은 기능을 재호출) 		//다시 검사하여
			}
			target.delete(); 		//삭제
		}
	}
